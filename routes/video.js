...
Router.get('/own-video',checkAuth,async (req,res)=>{
    ...
    try{
        ...
        const user = await jwt.verify(token,'sahil')
        console.log(user)
...
Router.put('/:videoId',checkAuth,async(req,res)=>{
    ...
    try{
        const token = req.headers.authorization.split(" ")[1]
        const verifiedUser = await jwt.verify(token,'sahil')
...
try{
    const verifiedUser=await jwt.verify(req.headers.authorization.split(" ")[1],'sahil')
